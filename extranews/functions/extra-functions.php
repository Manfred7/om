...
if ( !function_exists( 'ag_featured_posts_layout' ) ) :
...
if ( !function_exists( 'ag_featured_index_layout' ) ) :
...
if ( !class_exists( 'Walker_Cat_Filter' ) ) :
...
if ( ! function_exists( 'ag_get_posts' ) ) :
	function ag_get_posts() {

		global $post, $reviewstyle;

		if ( ! wp_verify_nonce( $_POST['nonce'], 'tw-ajax-nonce' ) ) {
			wp_send_json_error( __( 'Nonce Failed.', 'framework' ) );
		}
		...
		foreach ( $posts as $post ) {
			...
			if ( has_post_thumbnail( $post->ID ) ) {
				...
				$entry['alt']    = get_post_meta( $thumb, '_wp_attachment_image_alt', true ); // Alt text of image
...
if ( !function_exists( 'ag_get_cats_nolink' ) ) :
...
if ( !function_exists( 'ag_review_post' ) ) :
...
if ( !function_exists( 'ag_review_post_home' ) ) :
	...
	function get_post_info($image_size, $id, $crop, $thumbnum) {
			...
			$i++;
				  ...
				  $full = wp_get_attachment_image_src($full[0], 'full', false);  // URL of Featured Full Image
				   ...
				   if ( $crop ) {  if ($crop == 'No Crop' || $crop == 'no') {
...
if ( !function_exists( 'the_previousnextlinks' ) ) :
...
if ( !function_exists( 'get_the_content_unformatted' ) ) :
	function get_the_content_unformatted($content){
		...
		$old_content = get_the_excerpt();
		$old_content = str_lreplace('Read More', '', $old_content);
		}
...
if ( !function_exists( 'get_the_content_unformatted_more' ) ) :
...
if ( !function_exists( 'ag_get_related_post_args' ) ) :
 ...
 *
 */
if ( !class_exists( 'T5_User_Profile_Addon' ) ) :
...
if ( !function_exists( 'init_profile_addons' ) ) :
...
if ( !function_exists( 'ag_parse_url' ) ) :
...
if ( !function_exists( 'ag_social_links' ) ) :

    ...
    jQuery(document).ready(function($){
		...
		<?php } if ($twitter) { ?>
		 $('.tw').koottam({
			...
			'service'		: 'twitter',
			'nameology'		: 'Followers',
			'count'			: <?php echo ($twitter_count) ? $twitter_count : '0'; ?>
        });
		<?php } if ($rss) { ?>
		$('.rss').koottam({
			'id'			: '<?php echo $rss; ?>',
...
<p>
    <label for="<?php echo $this->get_field_id( 'twitter_count' ); ?>">
        <?php _e('Follower Count:', 'framework'); ?>
    </label>
    <input class="widefat" id="<?php echo $this->get_field_id( 'twitter_count' ); ?>" name="<?php echo $this->get_field_name( 'twitter_count' ); ?>" value="<?php echo $instance['twitter_count']; ?>" />
</p>
<p>

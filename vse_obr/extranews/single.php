...
<?php 
  ...
  $default_style = of_get_option('of_author_style') ? of_get_option('of_author_style') : 'avatar_box';
  $share_style = get_post_meta(get_the_ID(), 'ag_share_style', true);  // check share style
  $default_share = of_get_option('of_share_style') ? of_get_option('of_share_style') : 'minimal';
  ...
  <div class="blogpost maincontent"><div class="clear"></div> <!-- for stupid ie7 -->
    <?php if (have_posts()) : while (have_posts()) : the_post();  ?>
      ...
      <div class="blogcontent"> 
        ...
        /* Author 
	      /*-----------------------------------------------------------------------------------*/
        if ($author_style == 'Avatar Box' || $author_style == 'avatar_box' || ($author_style == '' && $default_style == 'avatar_box') ) : ?>
          <div class="details">
          ...
          /* Author
		      /*-----------------------------------------------------------------------------------*/ 
            if ($author_style == 'Simple' || $author_style == 'simple' || ($author_style == '' && $default_style == 'simple') ) : ?>
                <?php $author_id = get_the_author_meta('ID');
                $author_name = get_the_author_meta('display_name'); 
                $author_url = get_the_author_meta('user_url');  ?>
                <meta itemprop="datePublished" content="<?php the_time('Y'); ?>-<?php the_time('m'); ?>-<?php the_time('d'); ?>">
                <p class="name vcard author" itemprop="author"><span class="fn"><?php the_author_posts_link();?></span> | <?php _e('On', 'framework');?> <span class="updated"><?php the_time(get_option('date_format')); ?></span></p>
  		      <?php endif; ?>     
       ...
       <!-- Tags
        ...
        <div class="clear"></div>
          ...
          if ($share_style != 'None' && $share_style != 'none') {
            if ($share_style == 'Box Count' || $share_style == 'box_count' || ($share_style == '' && $default_share == 'box_count') ) {
